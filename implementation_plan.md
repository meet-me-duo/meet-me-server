...
- **PR 작성:** 사람이 읽고 검토하는 흐름을 우선한 위험 기반형 템플릿을 사용한다. 공통 체크리스트 대신 목적·변경 내용·검증·리뷰 요청·영향 범위를 구체적으로 적고 번역투나 과장된 표현을 피한다.
...
- **인증 경계:** 주최자는 Google·Kakao 로그인이 필수이고 일반 참여자는 게스트로 참여할 수 있다. 서비스 인증은 15분 RS256 JWT Access Token과 미사용 14일·패밀리 최대 30일의 Redis TTL 기반 회전형 불투명 Refresh Token을 사용한다. Access는 로그인·갱신 body로 발급한 뒤 `Authorization: Bearer` 헤더로 받고, Refresh는 같은 상위 사이트의 프론트엔드·API 배치를 전제로 API 호스트 전용 `Secure HttpOnly; SameSite=Lax; Path=/api/auth` 쿠키의 `Set-Cookie` 헤더로만 전달한다. Refresh·로그아웃은 명시적 프론트엔드 Origin을 엄격히 검증한다. 일반 로그아웃은 현재 패밀리만, 별도의 모든 기기 로그아웃은 사용자 전체 패밀리를 폐기하며 기존 Access Token은 최대 15분간 유효할 수 있다. 서비스 로그아웃은 공급자 연결을 해제하지 않는다. Refresh 원문은 저장하지 않고 해시·토큰 패밀리·폐기 상태만 Redis에 둔다. 공급자 Refresh Token은 별도로 PostgreSQL에 암호화 저장한다.
- **Calendar 접근:** Google Calendar 연동은 meet-me에 로그인한 개인에게만 제공한다. 개인 계정의 연결·일정 조회와 현재 참여 중인 방의 적용을 분리하고, Calendar를 연결한 참여자 본인이 현재 방에서 ON한 경우에만 불가 일정을 차감한다. ON은 불가 일정 외 탐색 범위를 허용한다는 뜻이므로 자연어 없이 제출할 수 있으며, 프론트엔드는 참여자·방별 최초 ON 때 확인 팝업을 한 번만 표시한다. Calendar ON 제출·수정 시점의 일정을 불변 제출 버전에 고정하고 방 마감에서는 다시 조회하지 않는다. OFF이면 불러온 일정도 무시한다. 비로그인 게스트는 OAuth를 사용할 수 없고 계정·다른 방에 재사용되지 않는 방 전용 시간 격자를 사용한다.
...
- **GitHub:** `origin`이 `https://github.com/meet-me-duo/meet-me-server.git`에 연결되어 있다. PR 기준점용 `main` 커밋 `64e5759`와 동일 기준의 `develop`이 원격에 생성되었다.
- **작업 트리:** 최신 `develop`에서 만든 `feature/initial-setup`에 초기 구성 전체를 복원했으며 동일 커밋과 PR로 반영할 예정이다.
- **초기 PR 방침:** 초기 구성은 `feature/initial-setup → develop` PR로 검토한다. 이후 모든 기능 작업은 최신 `develop`에서 새 feature 브랜치를 만들어 진행한다.
...
- [ ] `ko-KR` 기본 message bundle, `Accept-Language` 해석과 fallback 기반을 구성한다.
...
- [ ] `[USER]` Gemini 연동 직전 Google 프로젝트·결제/할당량을 확인하고 API Key를 발급하여 합의된 실행 환경에 직접 등록한다.
- [ ] `[USER]` Google OAuth 구현 직전 동의 화면과 앱 정보를 승인하고 Client ID/Secret을 발급하여 합의된 실행 환경에 직접 등록한다.
- [ ] `[USER]` Kakao OAuth 구현 직전 개발자 앱과 동의 항목을 승인하고 Client Secret을 합의된 실행 환경에 직접 등록한다.
- [ ] `[USER]` CI/CD 연동 직전 실제 자격 증명을 GitHub Repository/Environment Secrets에 직접 등록한다.
- [ ] `[USER]` AWS 인프라 적용 전 계정·조직·결제·예산·관리자 권한과 운영 배포를 승인한다.
- [ ] `[USER]` 관측성 연동 직전 Grafana Cloud 계정·stack·요금제와 알림 연락 채널을 선택하고 telemetry 전송 자격 증명을 합의된 실행 환경에 직접 등록한다.
- [ ] `[AGENT]` 각 사용자 작업 전에 필요한 secret 이름, 최소 권한, 서비스와 메뉴 위치, 단계별 행동, 공유 금지 값과 완료 확인 방법을 안내한다.
...
- [x] 주최자는 Google·Kakao 로그인을 필수로 하고 일반 참여자는 서비스 로그인 없이 게스트 참여를 허용하도록 확정한다.
...
- [ ] 지도·지오코딩 공급자 후보의 정확도, 비용, 할당량과 약관을 비교한다.
...
- [ ] 사용자, 소셜 계정, 게스트 브라우저 세션과 방별 참여자 소유권, 모임, 조건, 분석 배치·시도, 후보, 최종 확정의 초기 스키마를 설계하고 검토받는다.
...
**관련 요구사항:** FR-002A, FR-002B, FR-002C, FR-007A, FR-009, FR-012  
**선행 조건:** DG-02, DG-03

- [ ] Google 로그인 adapter 계약 테스트를 작성하고 구현한다.
- [ ] Kakao 로그인 adapter 계약 테스트를 작성하고 구현한다.
- [ ] 공급자 사용자 식별자와 서비스 사용자 연결 유스케이스를 구현한다.
- [ ] 선택한 인증 상태 및 토큰 발급·갱신·폐기 흐름을 구현한다.
- [ ] 일반 로그아웃의 현재 패밀리 폐기·쿠키 만료와 다른 기기 유지 계약을 테스트부터 구현한다.
- [ ] 모든 기기 로그아웃의 사용자 전체 패밀리 폐기·현재 쿠키 만료와 공급자 grant 유지 계약을 테스트부터 구현한다.
- [ ] 로그인한 주최자만 방 생성·재분석·확정을 수행하도록 권한 검증 유스케이스를 구현한다.
- [ ] 선택한 게스트 본인 증명 수단의 발급·검증·만료·회수 흐름을 구현한다.
- [ ] 게스트 자격 증명을 `Set-Cookie`로만 전달하고 응답 JSON·URL·프론트엔드 저장소에 노출하지 않는 계약 테스트를 작성한다.
- [ ] PostgreSQL에 게스트 자격 증명 원문이 아닌 단방향 해시와 만료·회수 상태만 저장하는 영속성 테스트를 작성한다.
- [ ] 하나의 게스트 브라우저 세션이 여러 방의 참여자를 소유하고 `(guest browser session, room)` 중복 참여는 기존 참여자로 귀결되는 영속성·동시성 테스트를 작성한다.
- [ ] 공유 초대 링크만으로 제출 조회·수정 권한을 얻지 못하는 보안 테스트를 작성한다.
- [ ] 인증 실패와 권한 오류의 공통 API 응답을 정의한다.
- [ ] Controller `@ApiResponse`와 DTO `@Schema`를 작성한다.
- [ ] 토큰, 공급자 응답과 개인정보가 로그에 남지 않는지 테스트한다.
- [ ] 인증 OpenAPI 계약과 통합 테스트를 검증한다.
...
- [ ] 로그인하지 않은 요청의 방 생성을 거부하고 인증된 주최자 계정에 방을 연결하는 테스트를 작성한다.
...
## Phase 5 — Google Calendar 불가 시간 수집

**관련 요구사항:** FR-003, FR-003A, FR-004, FR-004B  
**선행 조건:** DG-05, Phase 3, Phase 4

- [ ] Calendar 조회 outbound port의 내부 모델과 실패 타입을 정의한다.
- [ ] Google Calendar adapter 계약 테스트를 작성한다.
- [ ] 합의한 기간의 이벤트를 조회하고 절대 불가 시간으로 변환한다.
- [ ] Calendar ON 제출·수정 시점에 조회한 정규화 불가 시간과 조회 완료 시각을 해당 불변 제출 버전에 저장한다.
- [ ] 종일 일정, 반복 일정, 취소 일정과 시간대 변환을 처리한다.
- [ ] Calendar 절대 시점을 UTC로 정규화하고 방 Zone ID 기준 표시·비교를 검증한다.
- [ ] Calendar 연결·일정 조회 상태와 방별 `schedule_input_mode`를 분리하고, OFF인 방에서는 불러온 Calendar 데이터를 매칭에 사용하지 않는지 검증한다.
- [ ] Calendar ON에서 일정이 없는 유효한 빈 `calendar_blocked_times` 스냅샷과 Calendar 연결·조회 실패를 구분한다.
- [ ] 입력 수집 종료와 방 전체 배치 고정에서는 Google Calendar를 호출하지 않고 최신 제출 버전의 스냅샷만 사용하는 테스트를 작성한다.
- [ ] 게스트의 Google Calendar OAuth 시작·콜백·토큰 저장을 거부하고 로그인한 참여자의 grant만 서비스 사용자에 연결하는 계약 테스트를 작성한다.
- [ ] 권한 철회, 만료 토큰, 할당량 및 일시 장애 처리를 구현한다.
- [ ] Calendar API DTO와 토큰이 도메인에 노출되지 않는지 검증한다.
- [ ] 관련 API와 OpenAPI 계약 테스트를 검증한다.

## Phase 6 — 참여자 조건 제출과 자연어 구조화

**관련 요구사항:** FR-004A, FR-004B, FR-005, FR-006, FR-007, FR-007A, FR-008, FR-008A, FR-008B, FR-008C, FR-009, FR-013, FR-014  
**선행 조건:** DG-06, Phase 2, Phase 4

- [ ] nullable `raw_text`, `schedule_input_mode`, `manual_available_times`, `calendar_blocked_times`, `additional_blocked_times`의 세부 JSON 요청 계약을 확정한다. `MANUAL_AVAILABILITY`는 자연어 또는 하나 이상의 가능 시간 중 하나를 요구하고 빈 가능 시간은 부가 제약 없음으로 처리한다.
- [ ] 자연어·Calendar ON·수동 가능 시간이 모두 없는 요청을 `SUBMISSION_INPUT_REQUIRED`로 거부하고 제출 버전과 완료 인원을 만들지 않는 API·영속성 테스트를 작성한다.
- [ ] `CALENDAR` 모드를 방별 Calendar ON 의사로 취급하고 유효한 Calendar 스냅샷이 있으면 자연어 없이 제출할 수 있으며 별도 팝업 확인 boolean을 요구하지 않는 계약을 구현한다.
- [ ] 명시 범위에서는 실제 날짜형만, 기본 범위에서는 주간 반복형만 수동 가능·추가 불가 시간으로 허용하는 검증을 구현한다.
...
- [ ] 본인 증명을 거쳐 자신의 nullable `raw_text`, 방별 Calendar ON/OFF를 나타내는 일정 입력 모드, 수동 가능 또는 Calendar·추가 불가 시간과 수정 가능 여부를 조회하는 API를 구현한다.
- [ ] 로그인 사용자와 게스트 모두 본인 입력만 조회하고 주최자·공유 링크·타인 자격 증명으로 다른 입력을 조회할 수 없는 계약·권한 테스트를 작성한다.
...
- [ ] 입력 수집 중 Calendar ON/OFF 변경을 새 제출 버전으로 저장하고 종료 시점의 최신 모드만 고정하는 테스트를 작성한다.
- [ ] Calendar ON 수정은 새 조회 결과를 새 불변 제출 버전에 저장하고 이전 스냅샷을 덮어쓰지 않는 테스트를 작성한다.
...
- [ ] 수동 슬롯 또는 Calendar ON만 있는 참여자는 AI 요청에서 제외하되 매칭에는 포함되고, 전원이 정형 일정만 제출한 방은 Gemini 작업을 만들지 않는지 테스트한다.
...
- [ ] 인증된 주최자의 재분석 요청이 같은 고정 배치에 새 Outbox 이벤트를 멱등하게 만들고 참여자 재입력을 요구하지 않는 API를 구현·테스트한다.
- [ ] 배치 응답의 불투명 입력 참조값과 개수가 요청과 정확히 일치하는지 검증한다.
- [ ] Gemini 결과를 서버 스키마로 재검증하고 잘못된 출력을 거부한다.
- [ ] 조건 단위 검증으로 유효한 조건과 Calendar 불가 시간은 유지하고 해석 불가능한 조건만 제외하는 테스트를 작성한다.
...
## Phase 7 — 결정론적 일정·장소 매칭

**관련 요구사항:** FR-004B, FR-010, FR-011, FR-011A  
**선행 조건:** DG-01, DG-07, Phase 1, Phase 6

- [ ] 시간 구간 정규화와 교집합 계산 테스트를 작성한다.
- [ ] `MANUAL_AVAILABILITY` 가능 시간이 있으면 자연어 또는 중립 기준 구간과 교차하고, 빈 배열이면 자연어 기준 구간을 유지하며, 자연어와 슬롯이 모두 없으면 제출을 거부하는 테스트를 작성한다. Calendar ON은 자연어가 없으면 탐색 범위 전체에서 공급자·추가 불가 시간 합집합을 차감하고, OFF이면 불러온 일정을 무시하는지도 검증한다.
- [ ] 자연어 요일·시간 조건을 방 시간대와 탐색 범위의 실제 날짜별 구간으로 확장하는 테스트를 작성한다.
- [ ] 참여자 중 한 명이라도 가진 Calendar 불가 시간을 우선 차감하고 잘린 구간의 모임 소요 시간을 재검증하는 테스트를 작성한다.
...
## Phase 11 — MVP 통합 검증과 출시 준비

- [ ] 호스트 로그인 → 방 생성 → 링크 공유 흐름을 E2E 검증한다.
- [ ] 게스트의 자연어 전용·수동 슬롯 전용·두 입력 조합 제출과 본인 최신 입력 복원 흐름 및 Calendar OAuth 거부를 E2E 검증한다.
- [ ] 로그인 참여자의 Calendar 연결 후 방별 OFF 상태에서는 일정 미반영, 최초 ON 확인 후 Calendar 전용 제출, ON/OFF 수정과 최신 모드 매칭 흐름을 E2E 검증한다.
- [ ] Calendar ON 제출 뒤 공급자 일정이 변경되어도 기존 제출 스냅샷은 바뀌지 않고, ON 상태로 다시 제출한 경우에만 최신 스냅샷이 사용되는지 E2E 검증한다.
...
- [ ] 미완료 TBD와 MVP 이후 항목을 별도 백로그로 이동한다.
...
| FR-001B 주최자 선택 탐색 범위와 기본 14일 표시 | Phase 4, 7 | 미착수 |
| FR-002 참여 링크 | Phase 4 | 미착수 |
| FR-002A 주최자 필수 Google·Kakao 로그인 | Phase 3, 4 | 미착수 |
| FR-002B 서비스 계정 연결 | Phase 3 | 미착수 |
| FR-002C 게스트 참여자 본인 증명 | Phase 3, 4, 6 | 미착수 |
| FR-003 로그인 사용자 Calendar 연동·게스트 방 전용 격자 | Phase 4, 5, 6 | 미착수 |
| FR-003A Calendar 연결과 방별 ON/OFF 적용 분리 | Phase 5, 6, 7 | 미착수 |
| FR-004 Calendar 불가 시간 변환 | Phase 5 | 미착수 |
| FR-004B Calendar ON 제출 시점 스냅샷 고정 | Phase 5, 6, 7 | 미착수 |
| FR-004A 수동 가능 시간·Calendar 추가 불가 시간 격자 | Phase 1, 4, 6, 7 | 미착수 |
| FR-005 선택적 자연어 조건과 슬롯 전용 제출 | Phase 6 | 미착수 |
| FR-006 위치 표현 분류와 검증된 장소 정규화 | Phase 6 | 미착수 |
| FR-006A 이동 제약·미확정 장소의 좌표 생성 금지 | Phase 6 | 미착수 |
| FR-007 1-Pass Payload | Phase 6 | 미착수 |
| FR-007A 본인 최신 제출 조회 | Phase 3, 6 | 미착수 |
| FR-008 방 전체 제출 배치별 논리 파싱 작업·입력 제한·기술 오류 재시도 | Phase 6, 9 | 미착수 |
| FR-008A 수집 종료 전 수정과 최신 버전 매칭 | Phase 6 | 미착수 |
| FR-008B 기술 실패 분석 지연·의미 실패 부분 결과 | Phase 6, 7, 8, 9 | 미착수 |
| FR-008C 주최자 방 단위 재분석 | Phase 3, 6, 9 | 미착수 |
| FR-009 참여자별 조건 비공개 | Phase 3, 6 | 미착수 |
| FR-010 완료 조건 후 매칭 | Phase 4, 6, 7 | 미착수 |
| FR-010A 주최자 수동 조기 마감 | Phase 4 | 미착수 |
| FR-010B 최소 2명 제출 전 후보 생성 금지 | Phase 4, 7 | 미착수 |
| FR-011 Plan A/B/C | Phase 7 | 미착수 |
| FR-011A Calendar 불가 시간 차감과 구조화 후보·자연어 요약 | Phase 7 | 미착수 |
| FR-012 주최자 최종 확정 | Phase 3, 8 | 미착수 |
| FR-012A 부분 결과의 무수정·무추가확인 원클릭 확정 | Phase 8 | 미착수 |
| FR-013 IANA Zone ID와 UTC 기반 시간 모델 | Phase 1, 4, 5, 6 | 미착수 |
| FR-014 i18n과 언어 중립 API 코드 | Phase 0, 3, 6 | 미착수 |
