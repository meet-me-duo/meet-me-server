...
- [ ] 자동 조건 전 수동 마감의 경고·재확인 API 계약을 선택한다.
...
- [ ] 게스트 cookie `SameSite`·`Domain`·`Path`·`Max-Age`를 선택한다.
- [ ] 게스트 자격 증명 digest, 회전·회수, CSRF 방어와 기기 변경·쿠키 삭제 시 복구 정책을 선택한다.
...
- [ ] 주최자·로그인 참여자·게스트 참여자의 세부 권한과 방 접근 오류 계약을 선택한다.
...
- [ ] Structured Output 스키마와 조건 단위 서버 재검증의 상세 규칙을 선택한다.
...
- [ ] 방의 최대 참여자 수와 배치 출력 크기의 추가 상한을 선택한다.
...
- [ ] 익명 브라우저 자격 증명의 발급·검증·만료·회수 흐름을 구현한다.
- [ ] 자격 증명을 `Set-Cookie`로만 전달하고 응답 JSON·URL·프론트엔드 저장소에 노출하지 않는 계약 테스트를 작성한다.
- [ ] PostgreSQL에 자격 증명 원문이 아닌 단방향 해시와 만료·회수 상태만 저장하는 영속성 테스트를 작성한다.
- [ ] 방 생성 세션에 첫 참여자와 `HOST` 역할을 원자적으로 연결한다.
- [ ] 같은 세션만 방 마감·재분석·후보 확정을 수행하는 권한 테스트를 작성한다.
- [ ] 하나의 익명 브라우저 세션이 여러 방의 참여자를 소유하고 `(browser session, room)` 중복 참여는 기존 참여자로 귀결되는 영속성·동시성 테스트를 작성한다.
- [ ] 공유 초대 링크, 누락·위조 쿠키와 다른 참여자의 세션으로 본인 입력 또는 주최자 권한을 얻지 못하는 보안 테스트를 작성한다.
- [ ] 익명 세션 cookie의 `SameSite`·`Domain`·`Path`·`Max-Age`, CSRF와 허용 Origin 계약을 확정하고 테스트한다.
- [ ] 공개 방 생성·제출·Gemini 재분석 endpoint의 요청 제한과 남용 방지 경계를 구현한다.
- [ ] 인증 실패와 권한 오류의 공통 API 응답, Controller `@ApiResponse`와 DTO `@Schema`를 작성한다.
...
- [ ] 모임 생성 inbound port와 애플리케이션 서비스를 테스트부터 구현한다.
- [ ] 로그인 없이 방 생성을 허용하고 생성 요청의 익명 브라우저 세션에 주최자 참여를 연결하는 테스트를 작성한다.
- [ ] 모임 목적, 소요 시간과 선호 방식 입력 검증을 구현한다.
- [ ] 방 생성 시 주최자를 첫 번째 참여자로 같은 트랜잭션에서 생성하고 예상 참여 인원에 포함하는 테스트를 작성한다.
- [ ] 예상 참여 인원을 설정할 때 2명 미만을 거부하는 검증 테스트를 작성한다.
- [ ] MVP 방 생성 시 `Asia/Seoul` Zone ID를 저장하고 다른 Zone 선택을 허용하지 않는 계약 테스트를 작성한다.
- [ ] `search_start_date`, `search_end_date`를 함께 선택하거나 함께 생략하도록 검증하고 한쪽만 전달하면 거부하는 테스트를 작성한다.
- [ ] 탐색 날짜를 생략하면 방 시간대 생성일 기준 `[today, today + 14 days)`를 저장하고 응답하는 테스트를 작성한다.
- [ ] 주최자 명시 범위와 기본 14일 적용이라는 탐색 범위 출처를 저장하고 방 응답에 제공하는 테스트를 작성한다.
- [ ] 탐색 범위가 0일 이하이거나 31일을 초과하면 거부하는 테스트를 작성한다.
- [ ] OpenAPI `@Schema`에 기본 14일 정책을 명시하여 프론트엔드가 방 생성 화면에 안내할 수 있게 한다.
- [ ] 수동 전용 또는 예상 참여 인원·제출 마감 자동 조건을 갖는 완료 정책을 구현한다.
- [ ] 자동 조건 전 수동 마감의 경고·재확인 및 주최자 권한을 구현한다.
- [ ] 충돌 가능성을 고려한 참여 링크/코드 생성 방식을 비교하고 선택한다.
- [ ] 참여 링크 생성과 조회 흐름을 구현한다.
- [ ] 방 참여 유스케이스와 중복 참여 정책을 구현한다.
- [ ] 일반 참여자가 서비스 로그인 없이 참여하고 자신의 제출 소유권 증명을 발급받는 흐름을 구현한다.
- [ ] 모임 생성·참여 Web DTO와 Controller를 구현한다.
- [ ] 성공 및 주요 오류 `@ApiResponse`, DTO `@Schema`를 작성한다.
- [ ] 도메인·애플리케이션·Web·영속성 테스트를 각각 검증한다.
- [ ] 생성된 OpenAPI 계약을 검증한다.
...
- [ ] 제출 MVP의 nullable `raw_text`와 `manual_available_times` JSON 요청 계약을 확정한다. 자연어 또는 하나 이상의 가능 시간 중 하나를 요구하고 빈 가능 시간은 부가 제약 없음으로 처리한다.
- [ ] 자연어와 수동 가능 시간이 모두 없는 요청을 `SUBMISSION_INPUT_REQUIRED`로 거부하고 제출 버전과 완료 인원을 만들지 않는 API·영속성 테스트를 작성한다.
- [ ] 제출 MVP API가 `CALENDAR`, `calendar_blocked_times`와 `additional_blocked_times`를 노출하거나 수락하지 않는 계약 테스트를 작성한다.
- [ ] 명시 범위에서는 실제 날짜형만, 기본 범위에서는 주간 반복형만 수동 가능 시간으로 허용하는 검증을 구현한다.
- [ ] 수동 격자의 연속 선택 구간 병합과 범위·요일·시작·종료 검증 계약을 확정한다.
- [ ] 구조화된 시간 조건, 특정 가능한 장소, 이동 제약과 미확정 장소의 내부 스키마 및 검증 규칙을 확정한다.
- [ ] 방 Zone ID와 제출 BCP 47 locale을 AI Adapter 문맥으로 전달하고 언어 중립 Structured Output으로 변환하는 계약을 테스트한다.
- [ ] 자연어 파싱 outbound port와 공급자 독립 실패 타입을 정의한다.
- [ ] nullable 자연어와 정형 일정 입력의 원본 제출 버전 및 최신 참조를 저장하되 Gemini를 호출하지 않고 즉시 응답하는 접수 API를 구현한다.
- [ ] 본인 증명을 거쳐 자신의 nullable `raw_text`, 수동 가능 시간과 수정 가능 여부를 조회하는 API를 구현한다.
- [ ] 익명 참여자는 본인 입력만 조회하고 주최자 역할·공유 링크·타인 자격 증명으로 다른 입력을 조회할 수 없는 계약·권한 테스트를 작성한다.
- [ ] 제출 접수 트랜잭션 커밋 시 고유 참여자 완료 인원을 증가시키고 예상 인원 도달 시 입력 수집을 종료하는 테스트를 작성한다.
- [ ] `N`번째와 후속 제출이 동시에 도착해도 정확히 `N`개의 고유 참여자 제출만 접수되고 나머지는 종료 오류가 되는 동시성 테스트를 작성한다.
- [ ] 입력 수집 종료 트랜잭션이 종료 시점의 최신 버전을 방 전체 제출 배치로 고정하고, 자연어가 있을 때만 논리 작업과 Outbox 이벤트를 하나 만드는 테스트를 작성한다.
- [ ] 수집 중 수정본을 불변 버전으로 저장하고 고유 참여자 완료 인원은 증가하지 않는 테스트를 작성한다.
- [ ] 입력 수집 종료 시점의 최신 버전만 매칭 입력으로 고정하고 이후 수정은 거부하는 테스트를 작성한다.
- [ ] 수정과 종료의 동시 요청에서 커밋 순서에 따라 수정본 포함 또는 종료 오류 중 하나만 성립하는 동시성 테스트를 작성한다.
- [ ] 이벤트 ID와 방 전체 제출 배치 ID만 전달하고 Worker가 PostgreSQL에서 원문을 조회하는 비동기 작업을 구현한다.
- [ ] Gemini adapter의 Structured Output 계약 테스트를 작성한다.
- [ ] 참가자 제출·수정 중에는 호출하지 않고 자연어가 있는 제출만 방 전체 논리 작업 하나에 포함되는지 테스트한다.
- [ ] 수동 슬롯만 있는 참여자는 AI 요청에서 제외하되 매칭에는 포함되고, 전원이 수동 슬롯만 제출한 방은 Gemini 작업을 만들지 않는지 테스트한다.
- [ ] 참가자별 500자·배치 합계 10,000자 경계와 초과 입력 무절단 거부를 테스트한다.
- [ ] 최초 1회와 최대 3회 기술 재시도, Full Jitter 범위, `Retry-After`, 호출별 15초·전체 60초 timeout을 가상 시계와 주입 가능한 난수로 테스트한다.
- [ ] 기술적 재시도 소진 시 입력과 고정 배치를 보존하고 `ANALYSIS_DELAYED`로 전이하며 후보와 주최자용 원문 공개를 만들지 않는 테스트를 작성한다.
- [ ] 방 생성 익명 세션의 재분석 요청이 같은 고정 배치에 새 Outbox 이벤트를 멱등하게 만들고 참여자 재입력을 요구하지 않는 API를 구현·테스트한다.
- [ ] 배치 응답의 불투명 입력 참조값과 개수가 요청과 정확히 일치하는지 검증한다.
- [ ] Gemini 결과를 서버 스키마로 재검증하고 잘못된 출력을 거부한다.
- [ ] 조건 단위 검증으로 유효한 자연어 조건과 수동 가능 시간은 유지하고 해석 불가능한 조건만 제외하는 테스트를 작성한다.
- [ ] 성공한 Gemini 응답의 일부 자연어가 의미 검증에서 미반영되면 조율 작업은 `COMPLETED`, 후보 집합은 `PARTIAL`, 공개 진행 상태는 `READY_WITH_WARNINGS`가 되는 테스트를 작성한다.
- [ ] `집/회사/학교 근처`가 이동 제약으로, 하나로 특정되지 않는 지명이 미확정 장소로 분류되는 테스트를 작성한다.
- [ ] LLM이 만든 좌표와 Geo Adapter가 고유성을 검증하지 못한 장소 결과를 거부하는 테스트를 작성한다.
- [ ] 원본 제출 메타데이터, 정형 조건과 지도 검색 정규화 스냅샷을 PostgreSQL에 저장한다.
- [ ] 다른 참여자에게 개별 조건이 노출되지 않는지 API 테스트한다.
- [ ] AI 호출 횟수, 토큰과 추정 비용 메타데이터를 기록한다.
- [ ] 민감한 원문·좌표·공급자 응답이 로그에 남지 않는지 테스트한다.
- [ ] Controller `@ApiResponse`, DTO `@Schema`와 OpenAPI 계약을 검증한다.
...
| FR-002C 익명 참여자 본인 증명 | Phase 3, 4, 5 | 미착수 |
...
| FR-010 완료 조건 후 매칭 | Phase 4, 5, 6 | 미착수 |
| FR-010A 주최자 수동 조기 마감 | Phase 4 | 미착수 |
| FR-010B 최소 2명 제출 전 후보 생성 금지 | Phase 4, 6 | 미착수 |
