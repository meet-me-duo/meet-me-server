...
- **현재 단계:** Phase 3~4 — Issue #12 구현·검증 완료, PR 생성 준비
- **제품 기능:** 30일 익명 세션과 방 생성·조회·참여·수동 마감, HOST 권한과 OpenAPI 계약 구현 완료
...
- **현재 작업:** Issue #12의 `feature/anonymous-room-lifecycle`에서 30일 고정 익명 세션, 128비트 초대 코드, 표시 이름, 방 생성·조회·참여·수동 마감과 공개 API 계약의 구현·검증을 완료했다. 요청 제한은 5번 브랜치, 배치·Outbox 결합과 데드라인 스케줄러는 3번 브랜치에서 구현한다. AWS 설정은 사용자 지시 전까지 보류한다.
